...
where `trigger` can be any dynamic symbol wrapped in [Offload](../../Dynamics/Offload.md), that will be changed each frame, it serves the purpose of a signal, that all calculations are done. `event` is a string representation of [`EventObject`](../Misc/Events.md#`EventObject`) to which a handler function, that recalculates `trigger` is assigned.
...
![](../../../../dancers-ezgif.com-video-to-apng-converter.png)
