...
### Optimized version
Since it relies on [AnimationFrameListener](frontend/Reference/Graphics/AnimationFrameListener.md), it runs as fast as possible, which might be an issue for a lot of those objects on the screen.

Just using [`SetInterval`](frontend/Reference/Misc/Async.md#`SetInterval`) is not an options, since we need something to remove this timer, when there is no visible widgets. 

:::note
[ArrangeSummaryBox](frontend/Reference/Decorations/ArrangeSummaryBox.md) is a wrapper over [ViewBox](frontend/Reference/Decorations/ViewBox.md), which has an event generator. A user can attach [`EventHandler`](frontend/Reference/Misc/Events.md#`EventHandler`) to it and check if a widget was destroyed or created.
:::
...
- [`SetInterval`](frontend/Reference/Misc/Async.md#`SetInterval`) drives the calculations with `100 ms` interval
- [TransitionDuration](frontend/Reference/Graphics/TransitionDuration.md) interpolates the results with `100 ms` window
- We listen an events of creation and destruction of widgets using `"Event"` option of [ArrangeSummaryBox](frontend/Reference/Decorations/ArrangeSummaryBox.md)
