...
4. Once `onStart` pattern was captured (can be a fragment event - see [animations](frontend/Advanced/Slides/Widgets%20and%20components/animations.md)), it reduces the statistical temperature to the minimum (`easeTask`) and then stops the animation after some time
